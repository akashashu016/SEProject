...
<table border=0 width=80% style="font-family:Bradley Hand ITC;font-size:25px;" align="center">
<%@page import="java.sql.*" %>
<%
String cate=request.getParameter("location");
 Class.forName("com.mysql.jdbc.Driver");
 Connection con = DriverManager.getConnection("jdbc:mysql://localhost:8083/evento","root","password");
 PreparedStatement pst=con.prepareStatement("select * from event_details where ECity=?");
...
PreparedStatement p=con.prepareStatement("select fpath from fileupload where eid=?");
p.setInt(1,eid);
ResultSet r=p.executeQuery();
r.next();
String fpath=r.getString(1);
%><tr>
	<th><img src="images/<%= fpath %>" width="400" height="300" style="border-radius:20px;" /></th>


	
<th style="color:white"><main style="font-size:30px;font-weight:bold;"><%= rs.getString(2) %></main><br><%= rs.getString(3) %><br><%= rs.getString(5) %>/-<br><a href="viewmaps.jsp?loc=<%= rs.getString(6) %><%= rs.getString(8) %><%= rs.getString(9) %>" target="_blank" style="border-bottom:1px solid white;color:white;"><%= rs.getString(6)%>,<%= rs.getString(8) %>,<%= rs.getString(9)%></a><br><button style="background-color: #008080;padding: 2px 8px;margin: 8px 0;border: none;cursor: pointer;width:80%;border-radius: 19px;"><a href="bookevents.jsp?eid=<%= rs.getInt(1) %>" style="color:white;" >Book Event</a></button></th>
	</tr>
	<%
 }
%>
