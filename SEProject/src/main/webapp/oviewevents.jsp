...
String user=(String)session.getAttribute("ONAME");
 PreparedStatement pst=con.prepareStatement("Select * from payments where oname=?");
...
<th style="color:white"><main style="font-size:50px;font-weight:bold;"><%= rs.getString(2) %></main><br><%= rs.getString(3) %><br><%= rs.getString(5) %>/-<br><a href="viewmaps.jsp?loc=<%= rs.getString(6) %><%= rs.getString(8) %><%= rs.getString(9) %>" target="_blank"><%= rs.getString(6)%>,<%= rs.getString(8) %>,<%= rs.getString(9)%></a><br></th>
	</tr><%
 }
%>
</table>
